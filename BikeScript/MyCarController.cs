...
public class MyCarController : MonoBehaviour
{
	...
	[SerializeField]
	private GameObject[] m_Bike = new GameObject[2];
	[SerializeField]
	private Vector3 m_CentreOfMassOffset;
	[SerializeField]
	private float m_MaximumSteerAngle = 20f;
	[Range(0, 1)]
	[SerializeField]
	...
	{
		//log path
		Debug.Log("config Path: " + Application.persistentDataPath);
		...
		try{
			...
			m_Topspeed = float.Parse (info [2].ToString ());
		}catch(Exception e){
	...
	Vector3 position;
	// move 函数是提供给外界的接口
	public void Move(float steering, float accel, float footbrake, float handbrake,float R_BikeSpeed)
	{
		//m_SteerAngle = steering * m_MaximumSteerAngle;
		m_SteerAngle = steering;
		m_Bike[0].transform.localRotation=Quaternion.Euler (new Vector3(0,m_SteerAngle,0)); 
		if(R_BikeSpeed!=0){
			for (int i = 0; i < 4; i++)
			{
			...
			m_Bike [0].transform.rotation = quat;
			m_WheelColliders[2].GetWorldPose(out position, out quat);
			m_Bike [1].transform.rotation = quat;
		}
		...
		SteerHelper();
		if(R_BikeSpeed!=0){
			ApplyDrive(accel, footbrake,R_BikeSpeed);
		}
		ApplyDrive_BySpeed(R_BikeSpeed);
		CapSpeed();
		//Set the handbrake.
	...
	private void ApplyDrive_BySpeed(float BikeSpeed){
		float speed = m_Rigidbody.velocity.magnitude;
		switch (m_SpeedType)
		{
		case SpeedType.MPH:
			...
			speed *= 2.23693629f;
			if (speed > BikeSpeed)
				m_Rigidbody.velocity = (BikeSpeed / 2.23693629f) * m_Rigidbody.velocity.normalized;
			break;

		case SpeedType.KPH:
			speed *= 3.6f;
			if (speed > BikeSpeed)
				m_Rigidbody.velocity = (BikeSpeed / 3.6f) * m_Rigidbody.velocity.normalized;
			break;
		}
	}

	private void ApplyDrive(float accel, float footbrake,float BikeSpeed)
	{
		float thrustTorque;
		...
		for (int i = 0; i < 4; i++)
		{
			if (Vector3.Angle(transform.forward, m_Rigidbody.velocity) < 90f)
			{
